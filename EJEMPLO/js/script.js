...
window.addEventListener('scroll', () => {
    if (window.scrollY > 20) {
        nav.classList.add('shadow-lg', 'shadow-blue-900/5', 'bg-slate-900/95');
        nav.classList.remove('bg-slate-900/80');
    } else {
        nav.classList.remove('shadow-lg', 'shadow-blue-900/5', 'bg-slate-900/95');
...
window.updateChart = function (metric) {
    ...
    ['hours', 'money', 'errors'].forEach(m => {
        ...
        // Reset base classes
        btn.className = 'px-4 py-2 rounded-lg text-sm font-semibold transition bg-slate-800 text-gray-400 hover:bg-slate-700';
        ...
        if (m === metric) {
            btn.classList.remove('bg-slate-800', 'text-gray-400', 'hover:bg-slate-700');
...
window.addEventListener('scroll', () => {
    ...
    // Optional: Subtle parallax for Stats cards (staggered)
    const stats = document.querySelectorAll('.progressive-blur');
    stats.forEach((stat, index) => {
        if (scrolled < 1000) {
            const speed = 0.05 + (index * 0.02); // Different speed for each card
